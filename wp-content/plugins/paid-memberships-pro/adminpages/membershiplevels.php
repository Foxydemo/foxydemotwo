...
<?php
	...
	if($action == "save_membershiplevel") {
		...
		$ml_image_Upload = isset($_REQUEST['cust_image'])?$_REQUEST['cust_image']: "";
	...
	<div>
		...
		<form action="" method="post" enctype="multipart/form-data">
			...
			<tbody>
				...
				<tr>
					<th scope="row" valign="top"><label for="custom-image"><?php esc_html_e('Image','paid-memberships-pro');?>:</label>
					</th>
					<td>
						<img src="" style="height:80px; width:80px;" id="custom_image"><br>
						<input type="hidden" name="cust_image" id="cust_image">
					</td>
					<td>
						<input type="button" value="image upload" name="image"id="txt_image"/>
					</td>
				</tr>	
				<tr>
		...
		jQuery(document).on("click","#txt_image",function(){
			var image = wp.media({
				...
				var image_data = uploaded_image.toJSON();
				 jQuery("#custom_image").attr("src",image_data.url);
				jQuery("#cust_image").val(image_data.url);
			});
